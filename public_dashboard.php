        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                if ($type === 'Triangle Test') {
                    ...
                    $hedonicCountQuery = $conn->prepare("SELECT COUNT(*) as completed FROM hedonic WHERE request_no = ?");
                    $hedonicCountQuery->bind_param("i", $request_no);
                    $hedonicCountQuery->execute();
                    $hedonicCountResult = $hedonicCountQuery->get_result();
                    $hedonicCountRow = $hedonicCountResult->fetch_assoc();
